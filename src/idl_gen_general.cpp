  ...
  code += "\n\n";
    ...
    void GenJNITemplate(ServiceDef &service_def, std::string &code) {
        ...
        for (auto it = service_def.calls.vec.begin();
            ...
            // Find struct_def by RPC request type and generate a helper method
            for (auto it_struct = parser_.structs_.vec.begin();
        ...
        for (auto it = service_def.calls.vec.begin();
             it != service_def.calls.vec.end(); ++it) {
            RPCCall &call = **it;
            code += "  private static native byte[] _" + call.name
                    + "(byte[] req);\n";
        }
        code += "}\n\n";
    }
